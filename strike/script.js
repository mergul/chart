(function () {
  ...
  var x = d3.scale.linear().domain([1900, 2013]).range([0, width]);

  var y = d3.scale.linear().domain([0, 9.5]).range([height, 0]);
  ...
  var pitchX = d3.scale.linear().domain([1920, 2013]).range([0, pitchWidth]);

  var pitchY = d3.scale.linear().domain([1, 4.7]).range([pitchHeight, 0]);

  var pitchXAxis = d3.svg
    .axis()
    .scale(pitchX)
    .orient("bottom")
    .ticks(10)
  ...
  var pitchChart = d3
    ...
    .attr(
      "transform",
      "translate(" + pitchMargin.left + "," + pitchMargin.top + ")"
    );
  ...
  var batX = d3.scale.linear().domain([1988, 2012]).range([0, batWidth]);

  var batY = d3.scale.linear().domain([0.125, 0.4]).range([batHeight, 0]);
  ...
  var batChart = d3
    ...
    .attr(
      "transform",
      "translate(" + batMargin.left + "," + batMargin.top + ")"
    );

  queue()
    .defer(
      d3.csv,
      "https://static01.nyt.com/newsgraphics/2013/03/21/strikeouts/e499df91886aef061eb10983588acb400c454a59/out3.csv"
    )
    .defer(
      d3.csv,
      "https://static01.nyt.com/newsgraphics/2013/03/21/strikeouts/e499df91886aef061eb10983588acb400c454a59/teams.csv"
    )
    .defer(
      d3.csv,
      "https://static01.nyt.com/newsgraphics/2013/03/21/strikeouts/e499df91886aef061eb10983588acb400c454a59/average-by-count.csv"
    )
    .await(ready);
  ...
  function ready(err, strikeouts, teams) {
        ...
        return {
          ...
          rank:
            values
              ...
              .indexOf(2012) + 1,
          lastYear: values.filter(function (d) {
            return d.year === 2012;
          })[0].kpg,
        };
      })
    ...
    var tableTitle = league.append("h5").text(function (d) {
      return d.key === "NL"
    ...
    var row = table
      ...
      .attr("class", function (d) {
        return (d.values.rank === 1 ? "g-record-row " : "") + "g-team " + d.key;
      })
      .on("click", function (d) {
        selectTeam(d.key === selectedTeam ? null : d.key);
      });
    ...
    row
      ...
      .text(function (d) {
        return teamByCode[d.key].name;
      });

    row.append("td").text(function (d) {
      return format(d.values.lastYear);
    });
    ...
    row
      ...
      .append("div")
      .attr("class", function (d) {
        return d.values.rank === 1 ? "g-record" : "";
      })
      .text(function (d) {
        return d.values.rank === 1
          ? "Record"
          : formatOrdinal(d.values.rank) + " most";
      });
    ...
    var highestLabel = svg
      .append("g")
      .attr("class", "g-mean-text g-hide-hover")
      .attr(
        "transform",
        "translate(" + x(highestYear) + "," + y(highestkpg) + ")"
      );
    ...
    highestLabel.append("circle").attr("class", "g-highlight").attr("r", 5.5);
    ...
    var lowestLabel = svg
      ...
      .attr(
        "transform",
        "translate(" + x(lowestYear) + "," + y(lowestkpg) + ")"
      );
    ...
    lowestLabel.append("circle").attr("class", "g-highlight").attr("r", 5.5);
    ...
    pitchCountFilter = strikeouts.filter(function (d) {
      return d.year >= 1920;
    });

    var pitchersBounds = d3.geom.polygon([
      [0, 0],
      [0, pitchHeight],
    ...
    avgByCountFilter = strikeouts.filter(function (d) {
      return d.year >= 1988;
    });
    var batting2StrikesBounds = d3.geom.polygon([
      [0, batY(0.25)],
      [0, batY(0)],
      [batWidth, batY(0)],
      [batWidth, batY(0.25)],
    ...
    drawLineChart(
      ...
      batHeight,
      2.4,
      batting2StrikesBounds
    );
    var battingBounds = d3.geom.polygon([
      [0, batY(0.4)],
      [0, batY(0.25)],
      [batWidth, batY(0.25)],
      [batWidth, batY(0.4)],
    ...
    batChart.append("text").attr("class", "selected-team").attr("y", 20);
    ...
    function selectTeam(code) {
      ...
      if ((selectedTeam = code)) {
        ...
        d3.selectAll(".g-legend .g-selected-legend text").text(
          teamByCode[code].name
        );
      }
      d3.selectAll(".g-team-chooser").property("value", code || "none");
      d3.selectAll(".g-legend a").style(
        "display",
    ...
    function drawLegend(container, headline) {
      var legend = container.append("div").attr("class", "g-legend");

      var avgLegend = legend.append("div").attr("class", "g-average-legend");

      var avgSvg = avgLegend
        .append("svg")
        .attr("width", 24)
        .attr("height", 4)
        .append("g")
      ...
      avgSvg.append("line").attr("x2", 20);

      avgSvg.append("circle").attr("r", 2);

      avgSvg.append("circle").attr("cx", 20).attr("r", 2);

      avgLegend.append("div").text("League average");

      var selectedLegend = legend
        .append("div")
        .attr("class", "g-selected-legend");

      var selectedSvg = selectedLegend
        .append("svg")
        .attr("width", 24)
        .attr("height", 4)
        .append("g")
      ...
      selectedSvg.append("line").attr("x2", 20);

      selectedSvg.append("circle").attr("r", 2);

      selectedSvg.append("circle").attr("cx", 20).attr("r", 2);

      var teamChooser = selectedLegend
        .append("select")
        .attr("class", "g-team-chooser");

      teamChooser.append("option").attr("value", "none").text("Choose a Team");
      ...
      teamChooser
        ...
        .attr("label", function (d) {
          return (
            (d.key === "NL" ? "National" : "American") + " League"
          ).toUpperCase();
        })
        .selectAll("option")
        .data(function (d) {
    ...
    function drawLineChart(
      ...
      var averageData = d3
        ...
        .rollup(function (yearobj) {
          return {
            avg: d3.mean(yearobj, function (d) {
              return parseFloat(d[attributeY], 10);
            }),
            totH: d3.sum(yearobj, function (d) {
              return parseFloat(d.H, 10);
            }),
          };
        })
        .entries(data);
      ...
      var averages = avgContainer
        .selectAll(".g-mean-circle")
        .data(averageData)
        .enter()
        .append("circle")
      ...
      team
        .append("path")
        .attr("d", function (d) {
          return teamLine(d.values);
        })
        .style("stroke", "white")
        .style("stroke-width", 3);

      team.append("path").attr("d", function (d) {
        return teamLine(d.values);
      });
      ...
      teamsContainer
        ...
        .data(
          d3.geom
            .voronoi(
              data.map(function (d) {
                return [x(d.year), y(d[attributeY]) + Math.random() - 0.5];
              })
            )
            .map(bounds.clip)
            .map(function (d, i) {
        ...
        .on("click", function (d, i) {
          selectTeam(d.data.franchise);
        });

      var hoverLabel = teamsContainer
        .append("g")
        .attr("class", "g-mean-text")
        .style("display", "none");

      var hoverTeam = hoverLabel.append("text").attr("dy", ".35em");

      var hoverNumber = hoverLabel
        .append("text")
        .attr("class", "small number")
        .attr("dy", ".35em");

      var hoverYear = hoverLabel.append("text").attr("dy", ".35em");
      ...
      function selectValue(d) {
        if (d) {
          var offset =
            averageData[averageData.length - (2012 - d.year) - 1].values.avg <
            d[attributeY]
              ? -28
              : +32;
          hoverLabel
            ...
            .attr(
              "transform",
              "translate(" + x(d.year) + "," + y(d[attributeY]) + ")"
            );
          hoverNumber.attr("y", offset - 12).text(function () {
