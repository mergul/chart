...
define([
  ...
  this.init = function (options) {
    ...
    _chartCanvas
      .select("#xAxis")
      .append("g") // where x-axis will be rendered
      .attr("class", "x axis");

    var timeLegend = _chartCanvas
      .append("text")
    ...
    _svgContainer // mouse event not working on _chartCanvas
      .on("mouseover", function () {
        ...
        if (mX > 0 && mY > 0 && mX < width) hoverLine.style("opacity", 1);
      ...
      .on("mousemove", function () {
        ...
        if (mX > 0 && mY > 0 && mX < width) {
          ...
          var nearestDateVal = minDistanceDate(
            _.map(_graphs, function (d) {
              return d.map[mX] ? d.map[mX].date : null;
            }),
            dt
          );
          var graphIdswithDataAtNearestDate = _.chain(_graphs)
            .filter(function (d) {
  ...
  function selectChart(d) {
    if (d.type == "analog") {
      ...
      chart = d3
        .digital()
        .height(graphHeight(d))
        .color(color)
        .y(function (t) {
          return t.State ? 1 : 0;
        }); // 0/1 generator as y function
    }
    if (d.type == "horizon") {
      var mean =
        d.data
  ...
  function graphHeight(d) {
    if (d.type == "analog") return logChartHeight;
    else if (d.type == "digital") {
      //calculate height
      var cnt = _.uniq(d.data, false, function (t) {
        return t.Channel;
      }).length;
  ...
  function getLookupMap(graph, xScale) {
    ...
    var data = _.chain(graph.data)
      .rest(startIndex)
      .initial(endIndex - startIndex)
      .value();
    ...
    _.each(d3.range(width), function (px) {
      ...
      var dataIndex =
        cursorIndex + _.sortedIndex(_.rest(dates, cursorIndex), dt.valueOf()); // assuming data is sorted
      if (dataIndex < data.length) {
        if (dataIndex > 0) {
          var left = moment(data[dataIndex - 1].DateTime),
            right = moment(data[dataIndex].DateTime);
          if (moment(dt).diff(left) < right.diff(dt))
            // if left is nearer
  ...
  this.render = function () {
    //data-bind
    var graphs = _chartCanvas.selectAll(".graph").data(_graphs, function (d) {
      return d.id;
    });
