define(["d3", "components/sl"], function (d3, sl) {
  ...
  sl.series.fibonacciFan = function () {
    ...
    var fibonacciFan = function () {
      var root = target.append("g").attr("class", "fibonacci-fan");
    ...
    function setFan() {
      if (
        xScale(locationOrigin.point.date) > xScale(locationTarget.point.date)
      ) {
      ...
      lineA.transition().attr("y2", finalY.a);
      lineB.transition().attr("y2", finalY.b);
      lineC.transition().attr("y2", finalY.c);
      fanArea.transition().attr("points", pointsFinal);
    ...
    function calculateY(originX, originY, targetX, targetY, finalX) {
      var gradient = (targetY - originY) / (targetX - originX),
        ...
        yB = gradient * 0.5 * (finalX - originX) + originY,
    ...
    fibonacciFan.update = function () {
      ...
      circleOrigin.attr("cx", originX).attr("cy", originY);
      lineSource.attr("x1", originX).attr("y1", originY);

      if (locationTarget) {
        var targetX = xScale(locationTarget.point.date),
          targetY = yScale(locationTarget.point[locationTarget.field]);

        circleTarget.attr("cx", targetX).attr("cy", targetY);
        lineSource
          .attr("x1", originX)
          .attr("y1", originY)
          .attr("x2", targetX)
